    ...
    return (
        <div className="b-side-container">
            <div className="c-daily">
                ...
                <div>{unixToDateTimeConverter(data.dt)}</div>
                <div> {data.weather[0].description.toUpperCase()} 
                    <img src={`http://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png`} alt="weather-icon" height="20px" /> </div>
                {/* <div> <span>Rain:</span> <span>{data.daily[0].pop}%</span></div> */}
                <div><span>Current: </span> <span>{data.main.temp} &#8457;</span></div>
                <div><span>Feels Like: </span> <span>{data.main.feels_like} &#8457;</span></div>
                <div><span>Today's High: </span> <span>{data.main.temp_max} &#8457;</span></div>
                <div><span>Today's Low: </span> <span>{data.main.temp_min} &#8457;</span></div>
            </div>
            <div className="c-moon">
                <div className="t-banner"> Sun Data </div>
                ...
                <div> <span>Sunrise:</span> <span>{unixToTimeConverter(data.sys.sunrise)} </span></div>
                <div> <span>Sunset:</span> <span>{unixToTimeConverter(data.sys.sunset)} </span></div>
            </div>
            <div className="c-night-stroll">
                ...
                <div><span>Visibility:</span> <span>{data.visibility * .001} km</span></div>

                <div><span>Current:</span> <span>{data.main.temp} &#8457;</span></div>
                <div><span>Wind Speed:</span> <span>{data.wind.speed} mph</span></div>
